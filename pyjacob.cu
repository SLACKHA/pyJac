	...
	double* dConc, size_t pitch2)
{
	if (T_ID < num)
	{
		...
		eval_conc(T[T_ID], pres[T_ID], mass_local, &mass_local[NSP - 1], &mw_avg, &rho, conc_local);
	...
	size_t pitch2, const double* pres_mod, size_t pitch3, double* spec_rates, size_t pitch4)
{
	if (T_ID < num)
	{
		...
		eval_spec_rates(fwd_local, rev_local, pres_mod_local, spec_rates_local, &spec_rates_local[NSP - 1]);
...
__global__
void k_dydt(const int num, const double t, const double* pres, const double* y, size_t pitch1, double * dy,
	size_t pitch2)
{
	if (T_ID < num)
	{
		...
		//leave this as NN so that a 0
		//is copied back
		//just for safety
		double dy_local[NN] = {0};
...
void cu_dydt (const int num, const double t, const double* pres, const double* y, double* dy) {
	...
	//run
	k_dydt<<<grid_num, TARGET_BLOCK_SIZE, SHARED_SIZE>>>(num, t, dP, dY, pitch1, dDy, pitch2);
...
__global__
void k_eval_jacob(const int num, const double t, const double* pres, const double* y, size_t pitch1, double * jac,
	size_t pitch2)
{
	if (T_ID < num)
	{
