...
cdef extern from "pyjacob.cuh":
    ...
    void cu_eval_spec_rates (const size_t num, const double* fwd_rates, const double* rev_rates, const double* pres_mod, double * sp_rates)
...
def py_eval_conc(np.ndarray[np.double_t] T,
            np.ndarray[np.double_t] pres,
            np.ndarray[np.double_t, ndim=2] mass_frac,
            np.ndarray[np.double_t] mw_avg,
            np.ndarray[np.double_t] rho,
            np.ndarray[np.double_t, ndim=2] conc):
    cdef size_type num
    try:
        num = T.shape[0]
    except:
        num = 1
    cdef np.ndarray[np.double_t, ndim=2] temp_mass = mass_frac.T
    cdef np.ndarray[np.double_t, ndim=2] temp_conc = conc.T
    cdef np.ndarray[np.double_t] temp_t = T.T
    cdef np.ndarray[np.double_t] temp_p = pres.T
    cdef np.ndarray[np.double_t] temp_mw = mw_avg.T
    cdef np.ndarray[np.double_t] temp_rho = rho.T
    cu_eval_conc(num, &temp_t[0], &temp_p[0], &temp_mass[0, 0], &temp_mw[0], &temp_rho[0], &temp_conc[0, 0])
    conc = temp_conc.T
    mw_avg = temp_mw.T
    temp_rho = rho.T