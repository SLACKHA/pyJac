...
def build_parser():
    ...
    parser = ArgumentParser(
        prog='pyjac',
        description='pyJac: Generates source code for analytical chemical Jacobians.',
    )
    parser.add_argument(
        '-v',
        '--version',
...
def get_parser():
    ...
    if args.lang not in utils.supported_langs:
        parser.exit(
            2,
            f'{parser.prog}: {args.lang} output is not implemented. '
    ...
    if not os.path.isfile(args.input):
        parser.exit(2, f'{parser.prog}: mechanism file not found: {args.input}\n')

    if args.thermo is not None and not os.path.isfile(args.thermo):
        parser.exit(
            2, f'{parser.prog}: thermodynamic database not found: {args.thermo}\n'
        )
