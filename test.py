...
class cupyjac_evaluator(cpyjac_evaluator):
    ...
    def update(self, index):
        ...
        if index % self.num_cond == 0 and index != 0 and self.cuda_state.shape[0] > 0:
            ...
            if num_eval != self.num_cond:
                ...
                self.test_conc = self.czeros((num_eval, gas.n_species))
                self.test_fwd_rates = self.czeros((num_eval,gas.n_reactions))
                self.test_rev_rates = self.czeros((num_eval,num_rev))
                self.test_pres_mod = self.czeros((num_eval,num_pdep))
                self.test_spec_rates = self.czeros((num_eval,gas.n_species))
                self.test_dydt = self.czeros((num_eval, gas.n_species + 1))
                self.test_jacob = self.czeros((num_eval,(gas.n_species) * (gas.n_species)))
            ...
            self.pyjac.py_cujac(num_eval, self.num_cond, pres, y, self.test_conc, self.test_fwd_rates,
                            self.test_rev_rates, self.test_pres_mod, self.test_spec_rates,
                            self.test_dydt, self.test_jacob)

            self.cuda_state = self.cuda_state[num_eval:, ]

            self.test_conc = self.reshaper(self.test_conc, (num_eval, gas.n_species), self.back_spec_map)
            self.test_fwd_rates = self.reshaper(self.test_fwd_rates, (num_eval, gas.n_reactions), self.back_rxn_map)
            self.test_rev_rates = self.reshaper(self.test_rev_rates, (num_eval, num_rev), self.back_rev_rxn_map)
            self.test_pres_mod = self.reshaper(self.test_pres_mod, (num_eval, num_pdep), self.back_pdep_map)
            self.test_spec_rates = self.reshaper(self.test_spec_rates, (num_eval,gas.n_species), self.back_spec_map)
            self.test_dydt = self.reshaper(self.test_dydt, (num_eval, gas.n_species + 1), self.back_dydt_map)
            self.test_jacob = self.reshaper(self.test_jacob, (num_eval, (gas.n_species) * (gas.n_species)))
    ...
    def __init__(self, build_dir, gas, state_data):
        ...
        y = self.cuda_state[:num_eval, [0] + [2 + x for x in self.fwd_spec_map]].flatten(order='f')\
                                                                   .astype(np.dtype('d'), order='c')
        self.pyjac.py_cujac(num_eval, self.num_cond, pres, y, test_conc, test_fwd_rates,
