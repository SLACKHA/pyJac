...
__device__ void get_rxn_pres_mod (const double T, const double pres, const double * __restrict__ C, double * __restrict__ pres_mod) {
  ...
  // reaction 0;
  pres_mod[INDEX(0)] = m + 1.4 * C[INDEX(0)] + 14.4 * C[INDEX(5)] - 0.17000000000000004 * C[INDEX(8)];

  // reaction 1;
  pres_mod[INDEX(1)] = m + 1.0 * C[INDEX(0)] + 5.0 * C[INDEX(5)] - 0.30000000000000004 * C[INDEX(8)];

  // reaction 5;
  pres_mod[INDEX(2)] = m - 1.0 * C[INDEX(3)] - 1.0 * C[INDEX(5)] - 1.0 * C[INDEX(8)];

  // reaction 10;
  pres_mod[INDEX(3)] = m - 1.0 * C[INDEX(0)] - 1.0 * C[INDEX(5)] - 0.37 * C[INDEX(8)];

  // reaction 13;
  pres_mod[INDEX(4)] = m - 0.27 * C[INDEX(0)] + 2.65 * C[INDEX(5)] - 0.62 * C[INDEX(8)];

  // reaction 20;
  thd = m + 1.0 * C[INDEX(0)] + 5.0 * C[INDEX(5)] - 0.30000000000000004 * C[INDEX(8)];
