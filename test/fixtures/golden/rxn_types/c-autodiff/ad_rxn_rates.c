...
void eval_rxn_rates (const adouble T, const adouble& pres, const adouble * __restrict__ C, adouble * __restrict__ fwd_rxn_rates, adouble * __restrict__ rev_rxn_rates) {
  ...
  rev_rxn_rates[0] = C[2] * C[4] * kf / Kc;

  //rxn 1
  fwd_rxn_rates[1] = C[2] * C[0] * exp(3.8262472284067424e+00 + 2.7 * logT - (3.1501544760183588e+03 / T));
  //rxn 2
  fwd_rxn_rates[2] = C[0] * C[4] * exp(1.2063356343328579e+01 + 1.51 * logT - (1.7260431074669282e+03 / T));
  //rxn 3
  fwd_rxn_rates[3] = C[5] * C[1] * exp(1.1723186390453424e+01 + 1.4 * logT - (9.0579521674649295e+03 / T));
  //rxn 4
  kf = exp(2.9181557920442486e+01 - 1.0 * logT);
  fwd_rxn_rates[4] = C[1] * C[2] * kf;
  if (T <= 1000.0) {
  ...
  Kc = 1.0000000000000000e+00 * exp(Kc);
  rev_rxn_rates[5] = C[5] * C[3] * kf / Kc;

  //rxn 9
  kf = exp(1.8683045008419857e+01 - (-8.2024789072043529e+02 / T));
  fwd_rxn_rates[9] = C[6] * C[6] * kf;
  if (T <= 1000.0) {
