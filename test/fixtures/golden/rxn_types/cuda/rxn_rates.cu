...
__device__ void eval_rxn_rates (const double T, const double pres, const double * __restrict__ C, double * __restrict__ fwd_rxn_rates, double * __restrict__ rev_rxn_rates, double * __restrict__ dot_prod) {
  ...
  //rxn 1
  fwd_rxn_rates[INDEX(1)] = shared_temp[threadIdx.x + 1 * blockDim.x] * C[INDEX(0)] * exp(3.8262472284067424e+00 + 2.7 * logT - (3.1501544760183588e+03 / T));
  //rxn 2
  fwd_rxn_rates[INDEX(2)] = C[INDEX(0)] * shared_temp[threadIdx.x + 2 * blockDim.x] * exp(1.2063356343328579e+01 + 1.51 * logT - (1.7260431074669282e+03 / T));
  //rxn 3
  fwd_rxn_rates[INDEX(3)] = C[INDEX(5)] * shared_temp[threadIdx.x + 3 * blockDim.x] * exp(1.1723186390453424e+01 + 1.4 * logT - (9.0579521674649295e+03 / T));
  //rxn 4
